...
После вставки скрипта контейнер начнёт загружать образ. Дождитесь окончания загрузки (статус `stopped`) и запустите вручную:

```
/container start [find hostname=<prefix>]
```

> Контейнер не запускается автоматически после первой установки — только при перезагрузке роутера (`start-on-boot=yes`). Первый запуск всегда ручной.
